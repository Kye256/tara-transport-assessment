...
def voc_from_iri(iri: float, vehicle_class: str) -> float:
    """
    Estimate VOC as a function of IRI for a given vehicle class.
    Linear interpolation between with-project (IRI 3) and without-project (IRI 14) rates.
    For future use when replacing the CBA engine with IRI-based costs.

    Args:
        iri: International Roughness Index (m/km)
        vehicle_class: One of 'Cars', 'Buses_LGV', 'HGV', 'Semi_Trailers'

    Returns:
        VOC in USD/vehicle-km
    """
    from config.parameters import VOC_RATES
    voc_good = VOC_RATES["with_project"].get(vehicle_class, 0.126)
    voc_bad = VOC_RATES["without_project"].get(vehicle_class, 0.180)
    f = max(0.0, min(1.0, (iri - 3.0) / 11.0))
    return voc_good + f * (voc_bad - voc_good)


def vot_from_iri(iri: float, vehicle_class: str) -> float:
    """
    Estimate VoT as a function of IRI for a given vehicle class.
    Linear interpolation between with-project (IRI 3) and without-project (IRI 14) rates.
    For future use.
    ...
    Returns:
        VoT in USD/vehicle-km
    """
    from config.parameters import VOT_RATES
    vot_good = VOT_RATES["with_project"].get(vehicle_class, 0.028)
    vot_bad = VOT_RATES["without_project"].get(vehicle_class, 0.040)
    f = max(0.0, min(1.0, (iri - 3.0) / 11.0))
    return vot_good + f * (vot_bad - vot_good)
...
def generate_cashflows(
    ...
    road_length_km: float = 10.0,
    construction_cost_total: float = 5_000_000,
    construction_years: int = 3,
    discount_rate: float = 0.12,
    analysis_period: int = 20,
    base_year: int = 2026,
    vehicle_split: Optional[dict] = None,
    rainfall_zone: str = "moderate",
    material_quality: str = "average",
) -> dict:
    """
    Generate year-by-year IRI-based cashflows for future Option A integration.
    Uses IRI curves to compute VOC/VoT at each year rather than fixed rates.

    Args:
        iri_current: Starting IRI
        surface_type: Mapped surface type
        adt: Base year ADT
        growth_rate: Annual traffic growth rate
        road_length_km: Road length in km
        construction_cost_total: Total construction cost in USD
        construction_years: Construction duration
        discount_rate: Economic discount rate
        analysis_period: Analysis period in years
        base_year: Starting calendar year
        vehicle_split: Dict of vehicle class proportions
        rainfall_zone: 'low', 'moderate', or 'high'
        material_quality: 'poor', 'average', or 'good'

    Returns:
        Dict with yearly_cashflows, npv, summary
    """
    from config.parameters import (
        VEHICLE_CLASSES, VOC_RATES, VOT_RATES,
        DEFAULT_CONSTRUCTION_PHASING, MAINTENANCE_COSTS, RESIDUAL_VALUE_FACTOR,
    )

    if vehicle_split is None:
        vehicle_split = {"Cars": 0.60, "Buses_LGV": 0.20, "HGV": 0.15, "Semi_Trailers": 0.05}

    k = compute_k(surface_type, adt, rainfall_zone, material_quality)
    cap_dn = IRI_CAP.get(surface_type, 20.0)
    cap_wp = IRI_CAP.get(surface_type, 16.0)
    post_iri = POST_CONSTRUCTION_IRI.get(surface_type, 3.0)
    wp_k = WITH_PROJECT_K.get(surface_type, 0.035)
    maint_iri = MAINTENANCE_RESET_IRI.get(surface_type, 4.0)

    iri_dn = predict_iri(iri_current, analysis_period, k, cap_dn)
    iri_wp, _ = predict_with_maintenance(
        iri_current, analysis_period, k,
        construction_years=construction_years,
        post_construction_iri=post_iri,
        with_project_k=wp_k,
        maintenance_interval=10,
    ...
    phasing = DEFAULT_CONSTRUCTION_PHASING
    cashflows = []
    total_npv = 0.0
    ...
    for t in range(analysis_period + 1):
        ...
        traffic = adt * (1 + growth_rate) ** t

        # Construction costs
        constr_cost = 0.0
        if t >= 1 and t <= construction_years:
            share = phasing.get(t, 0.0)
            constr_cost = construction_cost_total * share

        # VOC/VoT savings from IRI difference
        voc_saving = 0.0
        vot_saving = 0.0
        for vc in VEHICLE_CLASSES:
            share = vehicle_split.get(vc, 0.0)
            vc_traffic = traffic * share * 365

            voc_dn = voc_from_iri(float(iri_dn[t]), vc)
            voc_wp = voc_from_iri(float(iri_wp[t]), vc)
            voc_saving += (voc_dn - voc_wp) * vc_traffic * road_length_km

            vot_dn = vot_from_iri(float(iri_dn[t]), vc)
            vot_wp = vot_from_iri(float(iri_wp[t]), vc)
            vot_saving += (vot_dn - vot_wp) * vc_traffic * road_length_km

        # Maintenance costs
        maint_dn = MAINTENANCE_COSTS["without_project"]["routine_annual"] * road_length_km
        maint_wp = MAINTENANCE_COSTS["with_project"]["routine_annual"] * road_length_km
        net_maint = maint_wp - maint_dn if t > construction_years else 0.0

        # Residual value in final year
        residual = 0.0
        if t == analysis_period:
            residual = construction_cost_total * RESIDUAL_VALUE_FACTOR

        net_benefit = voc_saving + vot_saving - constr_cost - net_maint + residual
        total_npv += net_benefit * df

        cashflows.append({
            "year_index": t,
            "calendar_year": year,
            "iri_do_nothing": round(float(iri_dn[t]), 2),
            "iri_with_project": round(float(iri_wp[t]), 2),
            "voc_saving": round(voc_saving),
            "vot_saving": round(vot_saving),
            "construction_cost": round(constr_cost),
            "net_maintenance": round(net_maint),
    ...
    return {
        ...
        "summary": {
            "iri_current": round(iri_current, 1),
            "iri_do_nothing_end": round(float(iri_dn[-1]), 1),
            "iri_with_project_end": round(float(iri_wp[-1]), 1),
        },
    }
