...
- System dependencies (list any found in step 7, e.g.):
  - `ffmpeg` — for video frame extraction (`brew install ffmpeg` on Mac, `sudo apt install ffmpeg` on Ubuntu)
  - List any others found during the system dependency check
